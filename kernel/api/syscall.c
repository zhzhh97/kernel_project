...
sys_read(read_args_t *arg)
{
        ...
        bytes = do_read(kern_args.fd, kern_args.buf, kern_args.nbytes);

        if (bytes < 0) {
                page_free(kern_args.buf);
                curthr->kt_errno = -bytes;
                dbg(DBG_PRINT, "(GRADING3D 1)\n");
                return -1;
        }

        if ((err = copy_to_user(arg->buf, kern_args.buf, bytes))< 0) {
                page_free(kern_args.buf);
                curthr->kt_errno = -err;
                dbg(DBG_PRINT, "(GRADING3D 1)\n");
                return -1;
...
sys_write(write_args_t *arg)
{
        ...
        bytes  = 0;
        while (bytes < kern_args.nbytes) {
                int write_bytes;
                
                if (kern_args.nbytes - bytes < PAGE_SIZE) {
                        write_bytes = kern_args.nbytes - bytes;
                        dbg(DBG_PRINT, "(GRADING3D 1)\n");
                } else {
                        write_bytes = PAGE_SIZE;
                        dbg(DBG_PRINT, "(GRADING3D 1)\n");
                }
                /*               if ((err = copy_from_user(buf, (void *) ((uint32_t) kern_args.buf + bytes), write_bytes)) < 0) {
                        page_free(buf);
                        curthr->kt_errno = -err;
                        //dbg(DBG_PRINT, "(GRADING3D 1)\n");
                        dbg(DBG_PRINT, "(GRADING3D 2)\n");
                ...
                copy_from_user(buf, (void *) ((uint32_t) kern_args.buf + bytes), write_bytes);

                int written_bytes = do_write(kern_args.fd, buf, write_bytes);

                if (written_bytes < 0) {
                        page_free(buf);
                        curthr->kt_errno = -written_bytes;
                        dbg(DBG_PRINT, "(GRADING3D 1)\n");
                        return -1;
                }
                bytes += written_bytes;
...
sys_getdents(getdents_args_t *arg)
{
        ...
        copy_from_user(&kern_args, arg, sizeof(getdents_args_t));

        dirent_t d;
        int i = 0;

        while (i < (int) (kern_args.count / sizeof(dirent_t))) {
                int dent = do_getdent(kern_args.fd, &d);
                if (dent < 0) {
                        curthr->kt_errno = -dent;
                        dbg(DBG_PRINT, "(GRADING3D 1)\n");
                        return -1;
                }
                if (dent == 0) {
                ...
                /*                if ((err = copy_to_user(kern_args.dirp + i, &d, dent)) < 0) {
                        curthr->kt_errno = -err;
                        //dbg(DBG_PRINT, "(GRADING3D 1)\n");
                        dbg(DBG_PRINT, "(GRADING3D 1)\n");
                        return -1;
                ...
                copy_to_user(kern_args.dirp + i, &d, dent);
