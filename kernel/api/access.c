...
{
        vmarea_t* vm_area = vmmap_lookup(p->p_vmmap, ADDR_TO_PN(vaddr));

        if(vm_area == NULL){
                dbg(DBG_PRINT, "(GRADING3D 1)\n");
                return 0;
        }
        int protection = vm_area->vma_prot;

        int mask_res = protection & perm;
        /* if (mask_res >= perm) {
                dbg(DBG_PRINT, "(GRADING3D 1)\n");
                return 1;
        } else {
...
range_perm(struct proc *p, const void *avaddr, size_t len, int perm)
{
        ...
        while(offset < len){
                ...
                if(!addr_perm(p, (void *)((uint32_t)avaddr + offset), perm)){
                        dbg(DBG_PRINT, "(GRADING3D 1)\n");
                        return 0;
                }
