...
kthread_create(struct proc *p, kthread_func_t func, long arg1, void *arg2)
{
	...
	dbg(DBG_PRINT, "(GRADING1A 3.a)\n");
        ...
        kthread_t* new_thread=slab_obj_alloc(kthread_allocator); //make new thread

        KASSERT(NULL != new_thread);
        new_thread->kt_state=KT_NO_STATE; //initialzed to no state
        new_thread->kt_cancelled=0;// initialzed the cancel state to not cancelled
        new_thread->kt_kstack=alloc_stack(); //allocate stack
        ...
        pagedir_t * pdir_proc=p->p_pagedir;
        list_insert_tail(&p->p_threads,&(new_thread->kt_plink));//add new thread to p.p_threads
        context_setup(&new_thread->kt_ctx,func,arg1,arg2,new_thread->kt_kstack,DEFAULT_STACK_SIZE,pdir_proc);
...
{
        KASSERT(!curthr->kt_wchan);//make sure that kthread is not blocked on any thread
	...
	panic("weenix returned to kthread_exit()!!! BAD!!!\n");
}
