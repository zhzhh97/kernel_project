...
pframe_get(struct mmobj *o, uint32_t pagenum, pframe_t **result)
{
        *result = pframe_get_resident(o, pagenum);
        if(*result) {
                while(pframe_is_busy(*result)) { //wait loop
                        sched_sleep_on(&(*result)->pf_waitq); 
                	dbg(DBG_PRINT, "(GRADING3A)\n");
		}   
        } else {
                *result = pframe_alloc(o, pagenum); //allocating new page
                    
                int ret_val = pframe_fill(*result); //filling new page
                if (ret_val < 0) { //unsuccessful fill
                        pframe_free(*result);
			dbg(DBG_PRINT, "(GRADING3D 2)\n");
                        return ret_val;
                }  
        }   
        KASSERT(NULL != *result); //required kassert
        dbg(DBG_PRINT, "(GRADING3A 1.a)\n");
	KASSERT(!pframe_is_busy(*result)); //required kassert
	dbg(DBG_PRINT, "(GRADING3A 1.a)\n");
        return 0; //success
}
...
{
        KASSERT(!pframe_is_free(pf)); //required kassert
	dbg(DBG_PRINT, "(GRADING3A 1.b)\n");
        KASSERT(pf->pf_pincount >= 0); //required kassert
	dbg(DBG_PRINT, "(GRADING3A 1.b)\n");
