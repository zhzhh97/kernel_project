...
special_file_read(vnode_t *file, off_t offset, void *buf, size_t count)
{
        ...
        dbg(DBG_PRINT, "(GRADING2A 1.a)\n");
        int bytes = file->vn_cdev->cd_ops->read(file->vn_cdev, offset, buf, count);
        dbg(DBG_PRINT, "(GRADING2B)\n");
        return bytes;
...
special_file_write(vnode_t *file, off_t offset, const void *buf, size_t count)
{
        ...
        int bytes = file->vn_cdev->cd_ops->write(file->vn_cdev, offset, buf, count);
        dbg(DBG_PRINT, "(GRADING2B)\n");
        return bytes;
}
...
special_file_mmap(vnode_t *file, vmarea_t *vma, mmobj_t **ret)
{
        ...
        bytedev_t *byte_dev = bytedev_lookup(file->vn_devid);

        int res = byte_dev->cd_ops->mmap(file, vma, ret);
