...
do_brk(void *addr, void **ret)
{
        ...
        vmmap_t *vm_map = curproc->p_vmmap;
        uint32_t start = ((uint32_t) curproc->p_brk - 1) / PAGE_SIZE + 1;
        uint32_t end = ((uint32_t) addr - 1) / PAGE_SIZE + 1;


        if (start > end) {
                vmmap_remove(vm_map, end, (start - end));
                dbg(DBG_PRINT, "(GRADING3D 1)\n");
        }

        if (start < end) {
                vmarea_t *vm_area = vmmap_lookup(vm_map, start - 1);
                int res = vmmap_is_range_empty(vm_map, start, (end - start));
