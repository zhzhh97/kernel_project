...
handle_pagefault(uintptr_t vaddr, uint32_t cause)
{
        ...
        vmarea_t* vm_area = vmmap_lookup(curproc->p_vmmap, ADDR_TO_PN(vaddr));
        ...
        if (!((cause & FAULT_WRITE) || (cause & FAULT_EXEC)) && !(vm_area->vma_prot & PROT_READ)){
                dbg(DBG_PRINT, "(GRADING3D 2)\n");
                do_exit(EFAULT);
        }

        if ((cause & FAULT_EXEC) && !(vm_area->vma_prot & PROT_EXEC)) {
        ...
        int res = pframe_lookup(vm_area->vma_obj, ADDR_TO_PN(vaddr) - vm_area->vma_start + vm_area->vma_off, forwrite, &pf);   
        ...
        if (res == 0) {
                ...
                uintptr_t phyic_addr = (uintptr_t)pt_virt_to_phys((uintptr_t)pf->pf_addr);
                int pt_res = pt_map(curproc->p_pagedir, (uintptr_t)PAGE_ALIGN_DOWN(vaddr), phyic_addr, pdFlag, ptFlag);
                dbg(DBG_PRINT, "(GRADING3D 1)\n");
        } else {
                dbg(DBG_PRINT, "(GRADING3D 2)\n");
                do_exit(EFAULT);
